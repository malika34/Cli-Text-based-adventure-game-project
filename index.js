    ...
    room: [
        {
            ...
            action: () => {
                console.log(chalk.green("You open the door and find yourself in a corridor."));
        ...
        },
        {
            ...
            action: () => {
                console.log(chalk.red("Nothing happens. You're still in the dark room."));
                chooseScenario(scenarios.room);
            }
        }
    ],
    corridor: [
        {
            ...
            action: () => {
                console.log(chalk.green("You climb the stairs and find yourself in a bright room."));
        ...
        },
        {
            ...
            action: () => {
                console.log(chalk.green("You walk down the hallway and find a locked door."));
    ...
    lockedDoor: [
        {
            ...
            action: () => {
                if (player.inventory.includes("key")) {
                    console.log(chalk.green("The door unlocks and you find a treasure chest! You win!"));
                    return;
                }
                else {
                    console.log(chalk.red("You don't have the key."));
                    player.health--; // Player loses health
                    console.log(chalk.red(`You lost 1 health point. Health remaining: ${displayHealth(player.health)}`));
                    if (player.health <= 0) {
                        endGame();
                    }
                    else {
...
function chooseScenario(choices) {
    inquirer
        .prompt([
        {
            type: "list",
        ...
        .then((answer) => {
        const chosenAction = choices.find((choice) => choice.description === answer.action);
        if (chosenAction) {
            chosenAction.action();
        }
        else {
...
console.log(chalk.blue("*               The Adventure                *"));
console.log(chalk.blue("*********************************************\n"));
console.log(chalk.yellow(`Welcome to The Adventure! You wake up in a dark room.
   _      _                         _
  | |    | |                       | |
  | |    | | ___  _ __   ___  _ __| |_
  | |    | |/ _ \\| '_ \\ / _ \\| '__| __|
